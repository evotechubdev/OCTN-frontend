...
function buildReportHtml(data) {
  ...
  const residentRows = (data.residents || []).filter((resident) => resident.name || resident.diagnosis).map((resident) => [resident.name, formatDate(resident.birthDate), resident.diagnosis, resident.bedridden, resident.weightLoss, resident.specialDiet, resident.priority]);
  const page5 = reportPage(data, "Seção 05", "Levantamento geral dos residentes", `<div class="metric-grid"><div class="metric"><span>Registros individualizados</span><strong>${residentRows.length}</strong></div><div class="metric"><span>Prioridade alta/imediata</span><strong>${highPriorityResidents}</strong></div><div class="metric"><span>Perda de peso informada</span><strong>${shown(data.health4Count, "—")}</strong></div><div class="metric"><span>Uso de suplemento</span><strong>${shown(data.health7Count, "—")}</strong></div></div>${reportTable(["Nome / identificação", "Data de nascimento", "Diagnóstico", "Acamado", "Perda de peso", "Dieta especial", "Prioridade"], residentRows)}<p class="report-note"><strong>Confidencialidade:</strong> esta página contém dados relacionados à saúde. O documento deve ser compartilhado apenas com pessoas autorizadas e armazenado conforme as políticas de privacidade aplicáveis.</p>`, 5, totalPages);
